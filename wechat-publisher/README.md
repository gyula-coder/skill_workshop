...
cp ~/.claude/skills/wechat-publisher/config.yaml.example ~/.claude/skills/wechat-publisher/config.yaml
# 编辑 config.yaml 填入公众号、生图、同步等配置
...
├── README.md                   # 项目说明
├── config.yaml                 # 统一配置(公众号/生图/同步,gitignored)
├── config.yaml.example         # 统一配置模板
├── .gitignore
├── scripts/
│   ├── publish.py              # 一键发布主流程(串联所有模块 + AI 味 gate)
│   ├── wechat_api.py           # 微信 API facade + CLI 入口(重导出 config/token/api)
│   ├── config.py               # config.yaml 加载 + ConfigError
...
`pyyaml` 是硬依赖(读 `config.yaml`),`requests` 用于 HTTP 调用:
...
3. 复制 `config.yaml.example` 到 `config.yaml` 并填入真实凭证和生图配置:
...
`config.yaml` 是唯一支持的配置文件,只从 skill 根目录读取。
...
默认 `baoyu-image-gen` 支持四种 provider,自动检测优先级为:

1. `evolink`
2. `ideogram`
3. `gemini-proxy`(Gemini CLI / chat 代理)
4. `openai`

生图后端选择有两种:

1. `baoyu-image-gen`:默认,支持 Evolink / Ideogram / Gemini proxy / OpenAI
2. `baoyu-danger-gemini-web`:Web 登录版 Gemini,使用 `scripts/baoyu_danger_gemini_web/`,需要本机已保存 Gemini Web 登录 cookie

所有生图凭证都放在 `config.yaml` 的 `image_generation` 下。
...
`publish.py` 会自动从 `config.yaml` 读 `author` / `theme`,再依次做图片处理 → HTML 转换 → AI 味检测 → 封面上传 → 创建草稿。默认阈值 45,过关才会继续。
...
作者和主题从 `config.yaml` 对应账号自动读取,也可通过 `--author` 覆盖。
...
- **`config.py`**:`config.yaml` 解析、`ConfigError`、`set_account` / `get_config` / `list_accounts`
...
基于 [`@wechatsync/cli`](https://github.com/wechatsync/Wechatsync) + Chrome 扩展,同步到知乎 / 掘金 / CSDN 等。建议在 `config.yaml` 的 `integrations.wechatsync_mcp_token` 设置 token。详见 SKILL.md 阶段七。
...
| 错误 | 原因 | 解决方法 |
|------|------|----------|
| `ConfigError: 未找到 config.yaml` | 配置文件不存在 | 复制 `config.yaml.example` 到 `config.yaml` 并填值 |
| `ConfigError: 账号 'xxx' 缺少 app_id` | 账号条目不完整 | 补齐 `app_id` / `app_secret` |
| `40164` IP不在白名单 | 机器 IP 未添加白名单 | `curl ifconfig.me` 获取 IP,添加到公众平台 |
| `40001` access_token无效 | 凭证错误或 token 过期 | 检查 `config.yaml` 中的 AppID/AppSecret |
