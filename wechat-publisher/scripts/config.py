...
def _load_unified_env() -> None:
    ...
    ideogram = image.get("ideogram") or {}
    if isinstance(ideogram, dict):
        _set_env_if_present("IDEOGRAM_API_KEY", ideogram.get("api_key"))

    evolink = image.get("evolink") or {}
    if isinstance(evolink, dict):
        _set_env_if_present("EVOLINK_API_KEY", evolink.get("api_key"))
        _set_env_if_present("EVOLINK_BASE_URL", evolink.get("base_url"))
        _set_env_if_present("EVOLINK_IMAGE_MODEL", evolink.get("image_model"))


def load_env() -> None:
    """
    从 config.yaml 加载辅助环境变量到 os.environ(不覆盖已有值)。
    """
    _load_unified_env()
...
def get_config(account_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "output_dir": yaml_config.get("output_dir", "") or "./output",
